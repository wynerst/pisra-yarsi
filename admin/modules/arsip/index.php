...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($title)) {
        ...
        $data['perihal'] = $dbs->escape_string($title);
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'Achive', $_SESSION['realname'].' update archive data ('.$data['title'].') with biblio_id ('.$_POST['itemID'].')');
            ...
            if ($insert) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'archive', $_SESSION['realname'].' insert archive data ('.$data['title'].') with biblio_id ('.$last_biblio_id.')');
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        if ($biblio_item_d[1] < 1) {
            if (!$sql_op->delete('biblio', "biblio_id=$itemID")) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'archive', $_SESSION['realname'].' DELETE Archive data ('.$biblio_item_d[0].') with biblio_id ('.$itemID.')');
                // delete related data
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if (!$in_pop_up AND $form->edit_mode) {
        $str_input = '<div class="makeHidden"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'arsip/pop_item.php?inPopUp=true&action=detail&biblioID='.$rec_d['biblio_id'].'\', 650, 400, \''.__('Items/Copies').'\')">'.__('Add New Items').'</a></div>';
        $str_input .= '<iframe name="itemIframe" id="itemIframe" class="borderAll" style="width: 100%; height: 70px;" src="'.MODULES_WEB_ROOT_DIR.'arsip/iframe_item_list.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>'."\n";
        $form->addAnything('Item(s) Data', $str_input);
    }
    // biblio authors
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'arsip/pop_author.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.__('Authors/Roles').'\')">'.__('Add Author(s)').'</a></div>';
        $str_input .= '<iframe name="authorIframe" id="authorIframe" class="borderAll" style="width: 100%; height: 70px;" src="'.MODULES_WEB_ROOT_DIR.'arsip/iframe_author.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
    ...
    // biblio topics
        $str_input = '<div class="'.$visibility.'"><a class="notAJAX"  href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'arsip/pop_topic.php?biblioID='.$rec_d['biblio_id'].'\', 500, 200, \''.__('Subjects/Topics').'\')">'.__('Add Subject(s)').'</a></div>';
        $str_input .= '<iframe name="topicIframe" id="topicIframe" class="borderAll" style="width: 100%; height: 70px;" src="'.MODULES_WEB_ROOT_DIR.'arsip/iframe_topic.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
    ...
    $str_input = '<div class="'.$visibility.'"><a class="notAJAX" href="javascript: openHTMLpop(\''.MODULES_WEB_ROOT_DIR.'arsip/pop_attach.php?biblioID='.$rec_d['biblio_id'].'\', 600, 300, \''.__('File Attachments').'\')">'.__('Add Attachment').'</a></div>';
    $str_input .= '<iframe name="attachIframe" id="attachIframe" class="borderAll" style="width: 100%; height: 70px;" src="'.MODULES_WEB_ROOT_DIR.'arsip/iframe_attach.php?biblioID='.$rec_d['biblio_id'].'&block=1"></iframe>';
