...
// privileges checking
$can_read = utility::havePrivilege('staf', 'r');
$can_write = utility::havePrivilege('staf', 'w');
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($memberID) OR empty($memberName)) {
        ...
        if (file_exists(MODULES_BASE_DIR.'staf/member_custom_fields.inc.php')) {
            include MODULES_BASE_DIR.'staf/member_custom_fields.inc.php';
        }
        ...
        if (isset($_POST['extend']) AND !empty($_POST['extend'])) {
            // get staff periode from database
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                if (isset($upload_status)) {
                    if ($upload_status == UPLOAD_SUCCESS) {
                        // write log
                        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', $_SESSION['realname'].' upload image file '.$upload->new_filename);
                        utility::jsAlert(__('Image Uploaded Successfully'));
                    } else {
                        // write log
                        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', 'ERROR : '.$_SESSION['realname'].' FAILED TO upload image file '.$upload->new_filename.', with error ('.$upload->error.')');
                        utility::jsAlert(__('Image FAILED to upload'));
                    }
                }
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', $_SESSION['realname'].' update member data ('.$memberName.') with ID ('.$memberID.')');
            ...
            if ($insert) {
                utility::jsAlert(__('New Staff Data Successfully Saved'));
                // upload status alert
                if (isset($upload_status)) {
                    if ($upload_status == UPLOAD_SUCCESS) {
                        // write log
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', $_SESSION['realname'].' add new staff ('.$memberName.') with ID ('.$memberID.')');
                echo '<script type="text/javascript">parent.$(\'#mainContent\').simbioAJAX(\''.$_SERVER['PHP_SELF'].'\');</script>';
            } else { utility::jsAlert(__('Staff Data FAILED to Save/Update. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        }
    }
    exit();
} else if (isset($_POST['batchExtend']) && $can_read && $can_write) {
    /* BATCH extend staff proccessing */
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        // write log
        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', $_SESSION['realname'].' extends staff for member ('.$mtype_d[1].') with ID ('.$memberID.')');
        $num_extended++;
    }
    header('Location: '.MODULES_WEB_ROOT_DIR.'staf/index.php?expire=true&numExtended='.$num_extended);
    exit();
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        if ($loan_d[2] < 1) {
            if (!$sql_op->delete('member', "member_id='$itemID'")) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'staff', $_SESSION['realname'].' DELETE staff data ('.$loan_d[1].') with ID ('.$loan_d[0].')');
            }
        } else {
...
<div class="menuBoxInner memberIcon">
    <?php echo strtoupper(__('Staff')); ?> - <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>staf/index.php?action=detail" class="headerText2"><?php echo __('Add New Staff'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>staf/index.php" class="headerText2"><?php echo __('Staff List'); ?></a>
    &nbsp; <a href="<?php echo MODULES_WEB_ROOT_DIR; ?>staf/index.php?expire=true" class="headerText2" style="color: #FF0000;"><?php echo __('View Expired Member'); ?></a>
    <hr />
    <form name="search" action="<?php echo MODULES_WEB_ROOT_DIR; ?>staf/index.php" id="search" method="get" style="display: inline;"><?php echo __('Member Search'); ?> :
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if ($form->edit_mode) {
        ...
        if ($is_expired) {
            ...
            $form->addCheckBox('extend', __('Extend Staff'), $chbox_array);
            $expired_message = '<b style="color: #FF0000;">('.__('Staff Already Expired').')</b>';
        }
    }
    ...
    $form->addSelectList('memberTypeID', __('Staff Type').'*', $mtype_options, $rec_d['member_type_id']);
    // member gender
    ...
    // member is_pending
    $form->addCheckBox('isPending', __('Pending Staff'), array( array('1', __('Yes')) ), $rec_d['is_pending']);
    // member photo
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('m.member_id',
            ...
            'mt.member_type_name AS \''.__('Staff Type').'\'',
    ...
    if ((isset($_GET['keywords']) AND $_GET['keywords']) OR isset($_GET['expire'])) {
        ...
        if (isset($_GET['expire'])) {
            echo '<b style="color: #FF0000;">'.__('Expired Staff List').'</b><hr size="1" />';
            echo '<div><input type="button" value="'.__('Extend Staff Member(s)').'" onclick="javascript: if (confirm(\''.__('Are you sure to EXTEND status for selected staff?').'\')) { $(\'#mainContent\').simbioAJAX(\''.MODULES_WEB_ROOT_DIR.'staf/index.php?expire=1\', { method: \'post\', addData: $(\'#memberList\').serialize() + \'&batchExtend=true\' } ); }" class="button" /></div>';
